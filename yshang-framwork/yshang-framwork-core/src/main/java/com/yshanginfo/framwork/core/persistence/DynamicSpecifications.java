 ...
 *******************************************************************************/
package com.yshanginfo.framwork.core.persistence;
...
import com.yshanginfo.framwork.core.utils.Collections3;
					...
					for (SearchFilter filter : filters) {
						...
						case ORLIKE:
							if (filter.value instanceof String) {
								...
								for (int i = 0; i < values.length; i++) {
									like[i] = builder.like(expression, "%"
											+ values[i] + "%");
								}
						...
						case ANDNOTLIKE:
							if (filter.value instanceof String) {
								...
								Predicate[] like = new Predicate[values.length];
								for (int i = 0; i < values.length; i++) {
									like[i] = builder.notLike(expression, "%"
											+ values[i] + "%");
								}
								predicates.add(builder.and(like));
							}
						...
						case OREQ:
							if (filter.value instanceof String) {
								...
								Predicate[] like = new Predicate[values.length];
								for (int i = 0; i < values.length; i++) {
									like[i] = builder.equal(expression,
											values[i]);
								}
								predicates.add(builder.or(like));
							}
						...
						case ANDNOTEQ:
							if (filter.value instanceof String) {
								...
								Predicate[] like = new Predicate[values.length];
								for (int i = 0; i < values.length; i++) {
									like[i] = builder.notEqual(expression,
											values[i]);
								}
								predicates.add(builder.and(like));
							}
